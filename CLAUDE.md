...
The short name on both sides is the safer variant; the deliberately verbose `PossiblyUnsafeHtml` name marks the unsafe path (React-`dangerouslySetInnerHTML`-style friction). Note: `|format` was previously the legacy alias for what is now `|formatPossiblyUnsafeHtml` — that alias was deleted in #747, and the name has now been reused for the safer variant, so PR/issue references predating #747 that mention `|format` mean the opposite of the current behavior.

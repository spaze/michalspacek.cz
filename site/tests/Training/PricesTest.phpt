...
class PricesTest extends TestCase
{
	...
	{
		$this->assertNoPrice(TrainingApplicationStatuses::STATUS_TENTATIVE);
	}


	public function testResolvePriceDiscountVatNoPriceNonPublicTraining(): void
	{
		$this->assertNoPrice(TrainingApplicationStatuses::STATUS_NON_PUBLIC_TRAINING);
	}


	public function testResolvePriceNoDiscountVat(): void
	{
		$price = $this->prices->resolvePriceDiscountVat($this->prices->resolvePriceVat(9990), 50, TrainingApplicationStatuses::STATUS_SIGNED_UP, 'foo');
	...
	{
		$price = $this->prices->resolvePriceDiscountVat($this->prices->resolvePriceVat(9990), 42, TrainingApplicationStatuses::STATUS_SIGNED_UP, 'FooStudentBar');
		...
		$price = $this->prices->resolvePriceDiscountVat($this->prices->resolvePriceVat(9990), 0, TrainingApplicationStatuses::STATUS_SIGNED_UP, 'Foostudent');
		Assert::same(9990.0, $price->getPrice());
		Assert::same(12087.90, $price->getPriceVat());

		$price = $this->prices->resolvePriceDiscountVat($this->prices->resolvePriceVat(9990), 100, TrainingApplicationStatuses::STATUS_SIGNED_UP, 'studentBar');
		...
		$price = $this->prices->resolvePriceDiscountVat($this->prices->resolvePriceVat(9990), 101, TrainingApplicationStatuses::STATUS_SIGNED_UP, 'studentBar');
		Assert::same(9990.0, $price->getPrice());
		Assert::same(12087.90, $price->getPriceVat());
	}

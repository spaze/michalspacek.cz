...
final class ValidationResultTemplateParametersTest extends TestCase
{
	...
	public function testStoresProperties(): void
	{
		...
		$params->fileErrors = [];
		$params->fileWarnings = [];
		$params->allRedirects = ['https://host.example/' => ['https://www.host.example/', 'https://www.host.example/.well-known/security.txt']];
		...
		Assert::same(['https://host.example/' => ['https://www.host.example/', 'https://www.host.example/.well-known/security.txt']], $params->allRedirects);
	}

...
final readonly class SecurityTxtValidator
{
	...
	private function checkHost(SecurityTxtValidatorUrl $url, ValidationResultTemplateParameters $template): void
	{
		...
		if ($result !== null && $this->addStoredAnswer($host, $result, $template, $now, $fetchAllowedIn)) {
			return;
		}

		if ($fetchAllowedIn !== null) {
			// Nothing fresh for this exact origin and no fetch allowed yet, so an answer that has merely gone stale is
			...
			if ($stale !== null && $this->addStoredAnswer($host, $stale, $template, $now, $fetchAllowedIn)) {
				return;
			}
	...
	/**
	 * Puts a stored answer on the page, whether it is still fresh or only the last thing this origin said. False when
	 * the row could not be read back, which is a miss rather than an error: the caller goes and asks the host again.
	 *
	 * @throws SecurityTxtFetcherException The stored answer, when what was stored was a failure. Thrown rather than
	 ...
	 * @throws CannotCreateDateTimeObjectException
	 */
	private function addStoredAnswer(
		string $host,
		Row $result,

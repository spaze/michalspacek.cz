...
final readonly class TrainingDateFactory
{
	...
	public function get(Row $row): TrainingDate
	{
		...
		return new TrainingDate(
			...
			$row->venueAddress,
			$row->venueCity,
			$row->venueDescription !== null ? $this->texyFormatter->format($row->venueDescription) : null,
			$row->note,
			$row->cooperationId,
			$row->cooperationDescription !== null ? $this->texyFormatter->format($row->cooperationDescription) : null,
			$row->price !== null ? $this->prices->resolvePriceVat($row->price) : null,
